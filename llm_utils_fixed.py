...
def _get_fallback_response(user_input: str, context: dict, intent: str) -> str:
    ...
    if intent == "rag":
        if any(word in text for word in ["hi", "hello", "hey", "help"]):
            return "Hi! Our platform helps creators automate content with AI. What are you interested in? (features, pricing, agents)"
        if "feature" in text or "platform" in text:
            ...
            return "Sure, tell me more about what you're interested in! (features, pricing, or getting started?)"
    else:
